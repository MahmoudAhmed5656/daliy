...
const firebaseConfig = {
  apiKey: "AIzaSyA6Opr42zOfyLeFvVS6ZcFS_z6KHN5jvbM",
  authDomain: "daily-e7902.firebaseapp.com",
  projectId: "daily-e7902",
  storageBucket: "daily-e7902.firebasestorage.app",
  messagingSenderId: "512855238306",
  appId: "1:512855238306:web:0bb1afea803137cec659fb",
  measurementId: "G-T7D9S75QH5"
};
...
async function saveAccountToFirebase(username, password) {
  ...
  saveAccountEncrypted(username, password);
}
...
async function verifyCredentials(username, password) {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      const decryptedPass = decryptData(userData.passwordHash);
      if (decryptedPass === p) return true;
    }
  } catch (err) {
    console.warn('Firebase connection warning, falling back to local state:', err);
  }

  // Fallback check in local accounts
  const accounts = getStoredAccounts();
  const found = accounts.find((a) => (a.username || '').trim().toLowerCase() === u);
  if (found) {
    return decryptData(found.passwordHash) === p;
  }
  return false;
}
...
function renderLoginForm() {
  ...
  if (state.isRegistering) {
    loginTitle.textContent = 'إنشاء حساب جديد للمنظومة';
    loginSubtitle.textContent = 'أدخل اسم المستخدم وكلمة المرور وكلمة سر المسؤول';
    submitBtnText.textContent = 'حفظ وإنشاء الحساب في Firebase';
...
function bindEvents() {
  ...
  loginForm?.addEventListener('submit', async (e) => {
    ...
    const adminPassInput = document.getElementById('login-admin-pass');
    const submitBtn = document.getElementById('login-submit-btn');
    const submitBtnText = document.getElementById('login-btn-text');

    const u = userInput.value.trim();
    const p = passInput.value;
    const ap = adminPassInput ? adminPassInput.value.trim() : '';
    ...
    const oldBtnText = submitBtnText ? submitBtnText.textContent : '';
    if (submitBtnText) submitBtnText.textContent = 'جاري التحقق...';

    try {
      if (state.isRegistering) {
        if (!ap) {
          errorAlert.textContent = 'برجاء إدخال كلمة سر المسؤول.';
          errorAlert.classList.remove('hidden');
          if (submitBtn) submitBtn.disabled = false;
          if (submitBtnText) submitBtnText.textContent = oldBtnText;
          return;
        }

        // Validate Admin Password against current hour and minute
        const now = new Date();
        ...
        if (!validTimePasses.includes(ap)) {
          errorAlert.textContent = 'كلمة سر المسؤول غير صحيحة.';
          errorAlert.classList.remove('hidden');
          if (submitBtn) submitBtn.disabled = false;
          if (submitBtnText) submitBtnText.textContent = oldBtnText;
          return;
        ...
        // Save account to Firebase Firestore
        await saveAccountToFirebase(u, p);
        successAlert.textContent = 'تم إنشاء الحساب وحفظه في Firebase بنجاح! يمكنك الآن تسجيل الدخول.';
      ...
      // Normal Login Mode - check Firebase
      const isValid = await verifyCredentials(u, p);
      if (isValid) {
        state.currentUser = u;
      ...
      errorAlert.textContent = 'حدث خطأ أثناء الاتصال بالخادم. يرجى المحاولة مرة أخرى.';
      errorAlert.classList.remove('hidden');
    } finally {
      if (submitBtn) submitBtn.disabled = false;
      if (submitBtnText && oldBtnText) submitBtnText.textContent = oldBtnText;
    }
  });
